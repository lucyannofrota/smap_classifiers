...
class yolo_v5(perception_wrapper):
    ...
    def __init__(self,detector_name='yolo_v5_odd'):
        super().__init__(
    ...
    def initialization(self):
        if super().initialization():  
            ...
            self.publisher_debug_image=self.create_publisher(Image, self.get_namespace()+'/perception/annotated_predictions/'+self.detector_name, 10,callback_group=self._reentrant_cb_group)
            return True
        return False
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    if len(det):
                        ...
                        for c in det[:, 5].unique():
                            jump = False
                            for j in range(8):
                                k = 2*j # Indexes to be rejected k*5 -> (k+1)*5-1
